...
try:
    ...
    VIDEO_TRANSCRIBER_IMPORT_ERROR = None
except ImportError as exc:
    video_transcriber = None
    VIDEO_TRANSCRIBER_IMPORT_ERROR = exc
...
class BilibiliSubtitleDownloader:
    ...
    def _download_single_subtitle(self, sub, page_title, format_type, video_dir):
        ...
        output_path = os.path.join(video_dir, f"{page_title}_{lan}.{format_type}")
        if os.path.exists(output_path) and os.path.getsize(output_path) > 0:
            print(f"✅ 发现本地已存在有效字幕文件: {os.path.basename(output_path)}")
            return output_path
        ...
        # 保存字幕文件
        return self._save_subtitle_file(subtitle_content, page_title, lan, format_type, video_dir)
    ...
    def download(self, video_url: str, video_index: str = "1", output_dir: str = 'subtitles',
                 ...
                 download_all_parts: bool = False, max_parts: Optional[int] = None,
                 start_index: Optional[int] = None, end_index: Optional[int] = None,
                 asr_model_size: str = "small", asr_device: str = "auto",
                 asr_compute_type: str = "auto") -> Dict[str, any]:
        """
        下载视频字幕和封面的主函数
        ...
        Args:
            ...
            max_parts: 下载所有分P时最多下载的分P/合集集数，None表示不限制
            start_index: 下载所有分P时的起始序号（从1开始）
            end_index: 下载所有分P时的终止序号（包含）
            asr_model_size: 本地ASR模型大小
            asr_device: 本地ASR运行设备
            asr_compute_type: 本地ASR计算类型
        ...
        # 根据 URL 参数、序号范围和 download_all_parts 兼容开关决定下载哪些分P。
        # 用户填写起始/终止序号时，直接按范围处理，不再需要额外的“全部分P”开关。
        range_requested = bool(start_index or end_index)
        if range_requested and len(pages) == 1:
            if self.debug:
                print("[DEBUG] 检测到单视频，忽略起始/终止序号范围")
            range_requested = False
        total_page_count = len(pages)
        if not download_all_parts and not range_requested:
            # 默认只下载 URL 指定的那一个分P；URL 未指定 p 参数时下载第一个。
            ...
            # 填了范围则按范围处理；没有范围但兼容开关开启时处理全部分P。
            if start_index or end_index:
                start = max((start_index or 1) - 1, 0)
                end = end_index if end_index and end_index > 0 else None
                pages = pages[start:end]
                if not pages:
                    print(f"错误: 选中范围内没有分P（该视频共 {total_page_count} 个分P）")
                    result['error'] = f"选中范围内没有分P（该视频共 {total_page_count} 个分P）"
                    return result
            if max_parts and max_parts > 0:
                pages = pages[:max_parts]
            if self.debug:
                mode_text = "按序号范围" if range_requested else "下载全部"
                print(f"[DEBUG] {mode_text}，处理 {len(pages)} 个分P")
        ...
        if not has_subtitle:
            if use_asr:
                ...
                import_error = f": {VIDEO_TRANSCRIBER_IMPORT_ERROR}" if VIDEO_TRANSCRIBER_IMPORT_ERROR else ""
                asr_errors.append(f"未检测到 video_transcriber 模块{import_error}")
                print(f"提示: 此视频没有官方/AI字幕，且未检测到 video_transcriber 模块，无法进行本地转录{import_error}")
            ...
            if not subtitles:
                ...
                if use_asr and video_transcriber:
                    ...
                    if video_transcriber.download_audio(
                        ...
                        print(f"启动独立进程进行转录 (Model: {asr_model_size}, Device: {asr_device}, Compute: {asr_compute_type})...")
                        sys.stdout.flush()
                        ...
                        try:
                            ...
                            transcribe_env.setdefault("PYTHONUNBUFFERED", "1")
                            ...
                            assert result_proc.stdout is not None
                            for output_line in result_proc.stdout:
                            ...
                            if os.path.exists(srt_path) and os.path.getsize(srt_path) > 0:
                                ...
                                if result_proc.returncode != 0:
                                    ...
                                    asr_errors.append(f"本地ASR转录失败，转录进程返回码: {result_proc.returncode}")
                                else:
                                    asr_errors.append("本地ASR转录失败，未生成SRT文件或文件为空")
                        except Exception as e:
                            print(f"调用转录进程失败: {e}")
                            asr_errors.append(f"调用转录进程失败: {e}")
                    else:
                        print("音频下载失败")
                        asr_errors.append("音频下载失败，无法进行本地ASR转录")
                
                continue

            if not result['subtitles'] and asr_errors:
                result['error'] = "；".join(asr_errors)

...
def _normalize_audio_path(path_value):
    return os.path.normpath(os.path.abspath(path_value))


def _finalize_audio_path(expected_path, output_path):
    expected_path = _normalize_audio_path(expected_path)
    output_path = _normalize_audio_path(output_path)
    if not os.path.exists(expected_path) or os.path.getsize(expected_path) <= 0:
        return False
    if os.path.normcase(expected_path) == os.path.normcase(output_path):
        return True
...
def _download_audio_via_bilibili_api(video_url, output_path, ffmpeg_path=None, cookies=None, headers=None, bvid=None, cid=None):
    ...
    try:
        ...
        if _finalize_audio_path(expected_path, output_path):
            print("B站API音频兜底下载成功")
            return True
...
def download_audio(video_url, output_path, ffmpeg_path=None, cookies=None, headers=None, bvid=None, cid=None):
    ...
    if bvid and cid:
        print("优先使用B站播放地址API下载音频...")
        if _download_audio_via_bilibili_api(
            video_url,
            output_path,
            ...
            return True
        print("B站播放地址API下载失败，尝试使用yt-dlp兜底...")
...
def transcribe_to_srt(audio_path, srt_path, model_size="small", device="auto", compute_type="auto"):
    """
    将音频转录为SRT字幕
    ...
    try:
        ...
        # 自动模式优先保证泛用性。部分 Windows CUDA/cuDNN 组合会在底层库初始化时
        # 直接终止进程，Python 无法捕获异常，因此只有用户明确选择 cuda 时才使用 GPU。
        if device == "auto":
            device = "cpu"
            print("自动模式使用CPU运行；如需GPU加速，请在设置中手动选择CUDA")
                
        # 根据设备调整compute_type
        if compute_type == "auto":
            compute_type = "float16" if device == "cuda" else "int8"
            print(f"自动选择计算类型: {compute_type}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--compute_type", default="auto", help="计算类型")
